...
<head>
    ...
    <style>
        ...
        .marker::after {
            ...
            pointer-events: none;
            display: none; /* Désactivé car on utilise border maintenant */
        }
...
<body>
    ...
    <script>
        ...
        console.log('========================================');
        console.log('🎯 DÉBOGAGE VODUN DAYS - ÉVÉNEMENTS');
        console.log('========================================');
        console.log('📊 Nombre d\'événements reçus:', events.length);
        console.log('📋 Liste des événements:', events);
        
        if (events.length === 0) {
            console.error('❌ AUCUN ÉVÉNEMENT REÇU !');
            console.error('Vérifiez les logs Laravel pour voir les erreurs API');
        } else {
            console.log('✅ Événements reçus avec succès');
        ...
        events.forEach(event => {
            ...
            // Ajouter classes spéciales pour VodunDays et événements en cours
            if (event.isVodunDays) {
                el.classList.add('vodundays-event');
            }
            ...
            // Ajouter badge "Démarre bientôt" si nécessaire
            ...
            markers.push({ marker, event, element: el });
        });
        ...
        function updateMarkerSizes() {
            ...
            // Calculer l'échelle en fonction du zoom (INVERSÉ)
            // Plus on dézoome (zoom petit), plus les marqueurs sont GRANDS (pour rester visibles)
            // Plus on zoome (zoom grand), plus les marqueurs sont PETITS (pour ne pas encombrer)
            // Zoom 8: scale 1.8 (180%)
            // Zoom 12: scale 1.4 (140%)
            // Zoom 16: scale 1.0 (100%)
            // Zoom 20: scale 0.6 (60%)
            let scale = 1.8 - (zoom - 8) * 0.1;
            scale = Math.max(0.4, Math.min(2.2, scale)); // Entre 40% et 220%
            
            console.log(`🔍 Zoom: ${zoom.toFixed(2)}, Scale: ${scale.toFixed(2)}`);
            
            markers.forEach(({ element }) => {
                if (element) {
                    // Appliquer la transformation avec !important via setProperty
                    ...
                    // Ajuster l'opacité pour les très petits zooms
                    if (zoom < 10) {
        ...
        // Écouter les changements de zoom en temps réel
        ...
        map.on('load', () => {
            setTimeout(() => {
                updateMarkerSizes();
                console.log('✅ Carte chargée et marqueurs initialisés');
            }, 100);
        });
